    ...
    if 'hotComments' in html:
        for item in html['hotComments']:
            ...
            if(item['content']==None):
                continue
            print("热门评论{}: {} ".format(m, item['content'].replace("\n"," ")))
            with open(filepath, 'a', encoding='utf-8') as f:
                f.write("{} {} {}\n".format(item['content'].replace("\n"," "),item['ipLocation']['location'], item['ipLocation']['userId'] ))
                # 回复评论
...
def comments(html, songname, i, pages, total, filepath): #获取其他评论 20条一页
    ...
    for item in html['comments']:
        ...
        if(item['content']==None):
            continue
        print("全部评论{} : {} ".format(j,  item['content'].replace("\n"," ")))
        with open(filepath, 'a', encoding='utf-8') as f:
            f.write("{} {} {}\n".format(item['content'].replace("\n"," "), item['ipLocation']['location'], item['ipLocation']['userId']))
            # 回复评论
            if len(item['beReplied']) != 0:
...
def main():
    ...
    songid = 2056970420
    # 歌曲名字
    songname = "shen ye dao li ming"
    # 评论文件存储路径
    filepath = "wyy/songs/" + songname + ".txt"
    print('开始爬取！')
